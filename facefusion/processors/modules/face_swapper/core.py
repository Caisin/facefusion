...
def process_frame(inputs : FaceSwapperInputs) -> ProcessorOutputs:
	...
	# multi-face mode: only when face_selector_mode is 'multi_reference'
	if face_selector_mode == 'multi_reference' and reference_face_paths and len(reference_face_paths) >= 1 and source_vision_frames and len(source_vision_frames) >= len(reference_face_paths):
	...
	# default single-source mode (many, one, reference)
